...
function buildAssDialogueLine(track, frame, nextFrame) {
  ...
  // Positional/short forms carry webOS-specific labels in the leading fields
  // (not reliable ASS Style/Name/Layer), so render with the always-present
  // Default style. Real positioning lives in \pos/\an tags inside the Text,
  // which are preserved. Only the Text and re-injected timestamps are kept.
